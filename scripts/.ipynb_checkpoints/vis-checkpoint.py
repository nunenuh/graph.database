...
def draw(graph, options, physics=False, limit=100):
    ...
    for row in data:
        ...
        if rel is not None:
            ...
            relation = get_rel(rel)
            
            edges.append({"from": source_info["id"], "to": target_info["id"], "label": relation[0]})
